...
A6. n is the number of Aperiodic linear granularities.
...
A7. $z$ is the original index for the bottom granularity $kin$. So, $C_{kin, uinal}(z) = z mod 20$. 

For $C_{katun,baktun}(z)$, we first need to have katun in terms of kin through the operation $18*20*z/20$

The following formula which expresses any circular granularity relating two linear granularities is utilized to come up with the equations:
$$
  C_{G, H}(z) = \lfloor z/P(B,G) \rfloor\mod P(G,H),
$$
$C_{katun,baktun}(z) = \lfloor z/P(kin,katun) \rfloor\mod P(katun,baktun)$

$C_{katun,baktun}(z) = \lfloor z/(20*18*20) \rfloor\mod 20$
and so on.
