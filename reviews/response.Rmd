...
A4. $w$ denotes the index set of $G'$.  $R'$ is the number of granules of $G'$ in each period.$T_w$ are the sets of indices of $B$ such that $G'(w) = \bigcup_{z \in T_w}B(z)$ does not use the definition of $T_w$. For example, if $B$ is `day` and $G'$ is `month`, then $G'(0) = {B(0)\cup B(2)\cup \dots \cup B(30)}$, $G'(1) = {B(31)\cup B(32)\cup \dots \cup B(58)}$ corresponding to January and February. Then $T_0 = \{0, 1, 2, \dots, 30\}$ and $T_1 = \{31, 32, \dots, 58\}$. If $z$ belongs to $k^{th}$ granule of $G'$, then $z \in T_k$. For example, if $z = 31$ then $z \in T_1$ and thus $k = 1$.
(Should this be added to paper?)
...
A5. R' is the number of granules of G' in the period P. For example, the grouping (day, month) has period 365 (ignoring leap years), then R = 12, since there are 12 months in 365 days. Given a period P, R' is constant.
...
Further, consider an example with  $H_3$ denoting "public holidays", then $G_1 \npreceq H_3$ and $G_1 \ntrianglelefteq H_3$.    
(Should this be added to paper?)
...
A3. Exactly equations (5) and (6) are to be utilized for this operation (with sides interchanged) and hence not added again.

Q4. Can the time granularity concept be extended to continuous time domain?

A4. For continuous time domains, the dependent variable is specified for every value of time $t$, whereas, in applications we generally have the values of the dependent variable for discrete time domain Hence, we will restrict ourselves to explore only discrete time domain. 
