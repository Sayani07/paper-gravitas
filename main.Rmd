...
1. Fix $k$. Then there are $L$ groups corresponding to level $A_k$ of $C_i$.
...
5. Use Steps 1-4 to compute normalised maximum distance for each $k$ levels ($NM_k$).

6. Compute $MMPD = median(NM_1, NM_2, \dots, NM_K)/log(K)$. Median normalised distance grows at the rate of log(K) as $K\rightarrow\infty$ and hence is normalised for the value of K in $MMPD$. 

7. Repeat Steps 1 to 6 for all harmony pairs.
