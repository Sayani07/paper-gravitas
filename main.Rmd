...
Periodicity is very common in all kinds of data. Time granularities which can accomodate periodicities can be constructed by relating two linear granularities. The mappings between linear granularities can be regular or irregular. For example, a regular mapping exists between minutes and hours, where 60 minutes always add up to 1 hour. On contrary, an irregular mapping exists between days and months, since one might can have days ranging from 28 to 31. Cyclic time granularities are referred to as **circular** if mappings between linear time granularities are regular and **quasi-circular** if the same is irregular. Examples of circular granularities include hour of the day, and day of the week, whereas examples for quasi-circular granularities can be day of the month or public holidays. To elaborate more, mappings will be regular if, for example, linear granularities are formed by grouping operation. Grouping operations combine fixed granules of a linear granularity to form a new granule of the resulting linear granularity. However, if granularities are formed by the altering-tick operation, for example, granules of the resulting granularity is composed of different number of granules of the root granularity. Both circular and quasi-circular granularities accomodate periodicities in data. However, the formalisms would differ.
...
and k(BG,G) be the period length of the bottom granularity with respect to linear granularity G. Here, linear granularities BG and G has regular mapping.
\end{timedom}
...
A **quasi-circular granularity** can not be defined using modular arithmetic due to its irregular mapping with the bottom granularity. For example, \autoref{tab:aperiodic} shows some example of quasi-circular granularities ($QC_i$) and plausible choices of number of granules of the finer granularity inside each granule of the coarser one($k_i$).
...
\begin{timedom}\label{def:quasi-circular}
A quasi-circular granularity QC_(BG, G) that relates linear granularities G and bottom granularity BG, with AG being periodical with respect to BG, is given by
...
$T_w$ are the sets of indices of BG describing $G(w)$ such that $G(w) = \bigcup_{z \in T_w}BG(z)$,$T_w = 0$ for $w \epsilon Z_{<0}$ and $\vert T_w \vert$ is the cardinality of set $T_w$. Here, linear granularities BG and G has irregular mapping.
\end{timedom}

Since G is periodical with respect to BG, from definition of "Groups periodically into" the following holds true:

- $BG \trianglelefteq G,$
- $G(w) = \bigcup_{z \in T_w \mod R}BG(P'*\lfloor w/R' \rfloor + z)$,  where P' denotes the period of the granularity BG.
...
Re-occurances in time can also happen due to completely irregular mapping between linear granularities. Some public and school holidays repeat every year, but there is no period within which their behavior remains constant. For example, U.S labour day is the first Monday in September and U.S. Memorial Day is the last day in May. A classic example can be that of Easter or street cleaning my old house occurs on the first and third Monday of the month between April and October, except for state holidays.As such all of these are still recurring events, but recurring with non-periodic patterns.

\begin{timedom}\label{def:acyclic}
An acyclic granularity AC_(BG, G) that relates linear granularities G and bottom granularity BG, with AG being periodical with respect to BG, is given by

\begin{equation}\label{eq:eq3}
QC_{BG, G}(z) = BG(z), z \in T_k
\end{equation}
where,  
z denotes the index set (index of BG),
$T_w$ are the sets of indices of BG describing $G(w)$ such that $G(w) = \bigcup_{z \in T_w}BG(z)$,$T_w = 0$ for $w \epsilon Z_{<0}$. Here, linear granularities BG and G has irregular mapping.
\end{timedom}
