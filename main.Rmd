...
1. Fix $k$. Then there are $L$ groups corresponding to level $A_k$ of $C_i$ for the pair $(C_i, C_j)$.

2. Compute  $m = \binom{L}{2}$ pairwise distances between distributions of $L$ unordered levels and $m = L-1$ pairwise distances for $L$ ordered categories.

3. Compute maximum within the $m$ distances.

4. Compute normalised maximum distance ($NM$) using appropriate norming constants so that the distribution of the normalised maximum converges to a Gumbel distribution as $m\rightarrow\infty$.

5. Use Step 1-4 to compute normalised maximum distance for each $k$ levels ($NM_k$).

6. Compute median maximum pairwise distance $$MMPD = median\{NM_1, NM_2, \dots, NM_K\}$$.

7. Repeat Step 1 to 6 for all harmony pairs.
