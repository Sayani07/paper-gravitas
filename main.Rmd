...
minute-of-day                                &
  ...
  $P_2= `r 60*24`$\\
hour-of-day                                  &
  $C_3 = L_3(\lfloor z/60\rfloor\mod 24)$    &
  $P_3 = \phantom{99}24$ \\
hour-of-week                                 &
  $C_4 = L_4(\lfloor z/60\rfloor\mod 24*7)$  &
  $P_4= \phantom{9}`r 24*7`$\\
day-of-week                                  &
  $C_5 = L_5(\lfloor z/24*60\rfloor \mod 7)$ &
  $P_5= \phantom{999}7$\\
\bottomrule
\end{tabular}
\end{center}
...
```{r quasi-circular-example, echo = FALSE, out.width = "100%",fig.cap="The time domain distributed as linear granularities $B$ (bottom granularity), $G'$, $H'$ and quasi-circular granularities $Q(B, G')$ and $Q(B, H')$. Both $Q(B, G')$ and $Q(B, H')$ have repeating patterns unlike $G'$ or $H'$ and the patterns are guided by the distribution of $B$ within $G'$ and $H'$ and remains constant within a period shown by the hashed arrows."}
...
```{r aperiodic-example, echo=FALSE,out.width = "100%",fig.cap="The time domain distributed as linear aperiodic granularities $M1$, $M2$, $M$ and aperiodic cyclic granularity $A(B, M)$ relating them with bottom granularity $B$. $A(B, M)$ have repeating patterns with three categories, two of which correspond to the aperiodic linear granularities $M1$ and $M2$ and other one correponding to no aperiodic granularity."}
...
smart_harmony <- smart_meter10 %>% 
  ...
  knitr::kable(smart_harmony,
               format = "latex",
               booktabs = TRUE,
                caption = "Harmonies with two cyclic granularity one placed on facet and the other on x-axis along with their number of categories/levels. Out of 30 possible combinations of cyclic granularities, only these 13 are harmony pairs.") %>% 
    #row_spec(0, bold = TRUE) %>% 
    kable_styling()
```
...
cust2_box <- smart_meter10 %>%
  ...
  xlab("") +
  ggtitle("") + ylab("") + 
  scale_x_discrete(labels = c('wdy','wnd'))
...
cricket_tsibble %>%
  ...
  mutate(inning = paste0("inning:", inning)) %>% 
  prob_plot("inning",
    "over",
    response = "runs_per_over",
    hierarchy_model,
