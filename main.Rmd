...
kable(table_mayan, 
  ...
  #row_spec(1:(nrow(table_mayan)-1), hline_after = T) %>% kable_styling(latex_options = "striped")
```
...
kable(table_greg, 
      format = "latex",
      booktabs = TRUE,
      caption = "Illustrative hierarchy table for Gregorian calendar with both circular and quasi-circular single-order-up granularities.") %>% 
  #row_spec(0, bold = TRUE) %>%
   kable_styling()
```
...
smart_harmony <- smart_meter10 %>% 
  ...
  knitr::kable(smart_harmony,
               format = "latex",
               booktabs = TRUE,
                caption = "Harmonies with two cyclic granularity one placed on facet and the other on x-axis.") %>% 
    #row_spec(0, bold = TRUE) %>% 
    kable_styling() %>% 
     add_footnote("NOTE: Out of 30 possible combinations of cyclic granularities, only these 13 are harmony pairs.",
                  notation="none")
```
