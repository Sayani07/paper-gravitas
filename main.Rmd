...
**Example**:  Consider the Gregorian calendar without leap year and all their exceptions. Then each month is a grouping of the same number of days over years. So the period is $P = 365$ and number of granules of "months" in each period is given by $R=12$. Thus, we could say days groups periodically into months. Now consider the Gregorian calendar with leap years but none of their exceptions. In such a case, if we consider every February $29^{th}$ as exceptions, we will still have each month as a grouping of the same number of days over years. In that case, we may say days groups quasi-periodically into months. The value of $P$ and $R$ would be 365 and 12 respectively while $E_i$'s would be the time domain corresponding to $29^{th}$ February of any year.
...
Here, g-rel could be any relationship defined between granularities like "groups into", "finer than" or "groups periodically". Given the set of all granularities defined over the time domain $(Z; <)$, and the granularity relationship $\trianglelefteq$ (groups into), the granularity mapping each index into the corresponding instant (same integer number as the index) is a bottom granularity with respect to $\trianglelefteq$.
...
The notion of labeled granularity is introduced in @Bettini2000-qk to simplify the calendar algebra. A labeled granularity is a pair $(\L, G)$, where $L$ is a subset of the integers, and $G$ is a mapping from $L$ to the subsets of the time domain such that (1) if $i<j$ , where $i$ and $j$ are in $L$, and $G(i)$ and $G(j)$ are non-empty, then each element in $G(i)$ is less than all the elements of $G(j)$, and (2) if $i<k<j$ , where $i$, $j$ , and $k$ are in $L$, and $G(i)$ and $G(j)$ are non-empty, then $G(k)$ is non-empty. $L$ is called the label set of $G$. When $L$ is exactly the integers, the granularities are called full-integer labeled granularity like the one defined in Definition \ref{def:linear}. However, the set $L$ of labels can be an arbitrary subset of (possibly non contiguous) integers and could be used to identify granules. Some calendar operations will only work on full-integer labeled granularities, while others could be implemented using more flexible labelings. 
...
Let us consider a granularity "pseudomonth" which computes months only for non-leap years.
\begin{align*}
