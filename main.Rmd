...
### Circular {#circular-gran-def}

In a cyclic organization of time, the domain is composed of a set of recurring time values. The formalism of time granularities in cyclic arrangement is attempted through the tsibble [@Wang2019-aw] framework of organizing temporal data. Suppose we have a tsibble with a time index in one column and keys and variables in other columns, a time domain, as defined by Bettini, is essentially a mapping of the index set to the time index. 

A linear granularity is a mapping of the index set to subsets of the time domain. For example, if the time index is days, then a linear granularity might be weeks, months or years. Since time is arranged in a cyclic way typically to accomodate periodicity, we will try to formalize how periodic linear granularities can be represented in a cyclic way. Since the assumption in [@Bettini2000-vy] is that all granularities can be generated from the bottom granularity by calendar algebraic operations - we extend this notion to relate index of the two granularities in a way, which represents the circular arrangement of time using modular arithmetic.
...
Suppose,
z denotes the index of the tsibble,
x,y are two units in the hierarchy table with  $order(x) <  order(y)$. 
Let $f(x, y)$ denotes the accessor function for representing the finer unit x in terms of a coarser unit y and $c(x, y)$ is a constant which relates x and y.
