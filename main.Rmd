...
hierarchy_model <- tibble::tibble(units = c("index", "ball", "over", "inning", "match"), convert_fct  = c(1, 6, 20, 2, 1))
hierarchy_model
```
...
cricket <-  read_csv("deliveries.csv")


# removing cases where length of overs were more or less than 6 
# extra run is wide_runs + noball_runs + bye_runs +  legbye_runs + penalty_runs

# cricket_filter <- data %>% 
#   filter(extra_runs == 0)%>%
#   group_by(batting_team, match_id, inning, over) %>% 
#   summarise(n = length(ball)) %>% 
#   filter(n==6) %>% 
#   mutate(key = paste(batting_team, match_id, inning, over, sep = "_"))
# 
# 
# cricket_periodic <- cricket %>% 
#   mutate(cricket_key = paste(batting_team, match_id, inning, over, sep = "_")) %>% 
#   filter(cricket_key %in% cricket_filter$key) %>% 
#   mutate(balls = purrr::rep_along(match_id, 1:6))


# 
# match_select <- cricket %>% 
#    filter(ball<=6) %>%
#   group_by(batting_team, match_id, inning)%>%
#   summarise(n = length(over)) %>% filter(n==120) %>% 
#   mutate(key = paste(batting_team, match_id, inning, over, sep = "_"))
# 
# cricket %>% 
#   group_by(match_id, inning, batting_team) %>% 
#   summarise(n_over = length(over))
...
cricketdata %>% mutate(data_index = row_number()) %>% as_tsibble(index = data_index) %>% dynamic_create_gran(hierarchy_model, "ball_over")
