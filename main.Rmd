...
<!-- What is multiple and single order up granularities -->
...
<!-- The index of a tsibble can be any date-time object which comes in different formats. [@Grolemund2011-qf] goes a long way in facilitating analysis of date-time data and extracting each date-time element with its own accessor function. However, table shows the accessor functions mostly allows for the creation of single-order-up granularities.  -->

<!-- ```{r table_accessor} -->
<!-- kable(tibble::tibble( -->
<!--   `Categorization` = c("Second-of-minute", "Minute-of-hour", "Hour-of-day", "Day-of-week", "Day-of-month", "Week-of-year", "Day-of-year","Month-of-year"), -->
<!--  Type = c("Periodic", "Periodic", "Periodic", "Periodic", "Aperiodic", "Aperiodic", "Aperiodic", "Periodic"), -->
<!-- Accessor = c("second()", "minute()", "hour()","wday()", "mday()","week()", "yday()", "month()")), format = "markdown") -->
<!-- ``` -->
...
### Periodic single-order-up granularities 
...
<!-- For example, if two linear granularties have three intermediate granularities in the hierarchical table, then their order difference is 3.  -->
...
f_{(x,y)}(z) & = f_{(x, x+1)}(z) + c(x, x+1)(f_{(x+1, y)}(z)-1) \\  
  ...
  &\vdots\\
  & = \sum_{i=0}^{order(y) - order(x) - 1} c(x, x+i)(f_{(x
  +i, x+i+1)}(z) - 1)\\
\end{split}
\end{equation}
...
```{r table_accessor1}
kable(tibble::tibble(
  `linear granularities`  = c("second", "minute", "hhour", "hour", "day", "week"),
  `Conversion factor` = c(60, 30, 2, 24, 7, 1), 
  `Single-order-up accessors` = c("second_minute", "minute_hhour", "hhour_hour", "hour_day", "day_week", 1)), format = "markdown")
```
...
f_{(hhour, week)}(z) & = f_{(hhour, hour)}(z) + c(hhour, hour)f_{(hour,day)}(z) + c(hhour, day)f_{(day, week)}(z) \\
              & = {hhour\_hour}(z) + 2{hour\_day}(z) + 2*24{day\_week}(z) \\
\end{split}
\end{equation}
...
```{r table_accessor2}
kable(tibble::tibble(
  `linear granularities`  = c("second", "minute", "hhour", "hour", "day", "month", "quarter"),
  `Conversion factor` = c(60, 30, 2, 24, "aperiodic", 3, 1), 
  `Single-order-up accessors` = c("second_minute", "minute_hhour", "hhour_hour", "hour_day", "day_month", "month_quarter", 1)), format = "markdown")
```

There can be three scenarios for obtaining calendar categorization here:
- calendar categorization consisting of two linear granularities whose orders are less than day  
...
For the purpose of this analysis. We only take the data for the year 2013. Let us look at the distribution of energy across coarser temporal granularities and then deep dive into finer temporal granularities.
...
```{r cust1_hd_my}
library(ggplot2)
data_sm1 %>% granplot("month_year", "hour_day", response = "general_supply_kwh", plot_type = "quantile", quantile_prob = c(0.1, 0.25, 0.5, 0.75, 0.9)) +  ggtitle("Letter value plot of energy consumption across hours of day by month of years for Household 1")+ scale_y_continuous(breaks = seq(0, 1.5, 0.25))
```


```{r cust3_hd_my}
data_sm3 %>% granplot("month_year", "hour_day", "general_supply_kwh", plot_type = "quantile", quantile_prob = c(0.1, 0.25, 0.5, 0.75, 0.9)) + ggtitle("Letter value plot of energy consumption across hours of day by month of years for Household 2") + scale_y_continuous(breaks = seq(0, 1.5, 0.25))
``` 

-  Hourly usage across days of the week

\autoref{fig:cust1_hd_my} shows distribution of hourly eenrgy usage for two households across different days of the week through decile plots. It is clear that household 1 deviates from their typical behavior only on weekends. A plot of the distribution across months of the year and days of the week might throw some light on the extreme behavior we saw in May and June. Was it only for the months May and June, that this household consumed more energy than they usually do? \autoref{cust1_dw_my} shows that it is not only those months that saw high energy consumption on Saturday and Sundays. In fact, months like November nad December saw higher ebergy usage than typical even on other weekdays. Still for this household, it can safely said that most of their activities which involve higher energy consumption are concentrated on weekends. The behavior of the weekends, as can be observed from the letter values of energy consumption, are different across months which can be attributed to weather conditions. 

/autoref{fig:cust3_dw_my} shows that the second household uniformly distribute their regular activity like using dish washer or heater/air conditioner throughout the week. The distribution of energy consumption on Monday, Friday and Sunday looks quite similar for December. Also, for other months, it is not very clear if weekend behavior varies distinctly from weekday behavior. 

```{r cust3_dw_my}
library(ggplot2)

data_sm1 %>% granplot("month_year", "day_week", response = "general_supply_kwh", plot_type = "lv", quantile_prob = c(0.1, 0.25, 0.5, 0.75, 0.9)) +  ggtitle("LV plot of energy consumption across hours of day by month of years for Household 1")+ scale_y_continuous(breaks = seq(0, 1.5, 0.25))
```
...
data_sm3 %>% granplot("month_year", "day_week", response = "general_supply_kwh", plot_type = "lv") +  ggtitle("LV plot of energy consumption across hours of day by month of years for Household 3")+ scale_y_continuous(breaks = seq(0, 1.5, 0.25))
```
...
```{r cust1_hour-day_day-week}
data_sm1 %>% granplot("day_week", "hour_day", "general_supply_kwh", plot_type = "decile") +  ggtitle("Boxplot of energy consumption for Customer 1")
```
...
``````{r cust3_hour-day_day-week}
data_sm3 %>% granplot("day_week", "hour_day", "general_supply_kwh", plot_type = "decile") + ggtitle("Boxplot of energy consumption for Customer 3")
```

-  Half-Hourly usage across days of the week

Now, let us check which half hours are the most active for Household 1. For most days, half hours between 34th and 38th  half hour of the day are higher than other days. The energy consumption in these hours on Fridays match with that os Saturday and Sunday.



<!-- ```{r cust1_hhour-day_day-week} -->
<!-- data_sm1 %>% -->
<!--    filter(lubridate::hour(reading_datetime)>=23) %>% -->
<!--   granplot("hhour_day","day_week", response = "general_sxupply_kwh", plot_type = "lv") + -->
<!--   ggtitle("Letter value plot of energy consumption across half-hour of the day across days of the week for Household 1") -->
<!-- ``` -->

<!-- <!-- # ```{r cust2_hhour-day_day-week} --> -->
<!-- <!-- # data_sm2 %>% granplot("day_week", "week_month", "general_supply_kwh", plot_type = "decile") + ggtitle("Boxplot of energy consumption for Customer 2") --> -->
<!-- <!-- # ``` --> -->

<!-- ```{r cust3_hhour-day_day-week} -->
<!-- data_sm3 %>% create_gran("hhour_day") %>% -->
<!--    filter(hhour_day>=23) %>% -->
<!--   granplot("hhour_day","day_week",response = "general_supply_kwh", plot_type = "lv") + -->
<!--   ggtitle("Letter value plot of energy consumption across half-hour of the day across days of the week for Household 1") -->
<!-- ``` -->
