...
\begin{example} 
{\rm Due to the uneven length of $month$, we need to use the altering-tick operation to compute $month$ from $day$. We can consider a granularity $pseudomonth$ which computes $month$ only for non-leap years by grouping 31 granules of $day$, and then shrink April, June, September and November by 1 granule of $day$, and shrink February by 3 granules of $day$ [@Ning_2002]. For more variations of calendar algebra operations, see @Ning_2002.
}
\end{example}
...
\begin{example} 
{\rm Let us use Equation \ref{eq:eq7} to compute the multiple-order-up granularity $C_{uinal,katun}$ for Mayan calendar with the index set  $Z=\{z: z \in \mathbb{Z}_{\geq 0}\}$ mapping "kins" to a set of positive integers.
...
\begin{example}
{\rm Considering the same example of Mayan Calendar, it is possible to compute the single-order-up granularity tun-of-katun given the multiple-order-up granularity uinal-baktun using equation \ref{eq:all-circular-multiple}.
