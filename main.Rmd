...
(in Figure \@ref(fig:cust1_hd_my)) shows the distribution of hourly comsumption of the households across different months of the year. The two households are similar when it comes to your usage in the morning hours. wThe second household has an evening peak across all months in contrast to the first household who has a steep evening peak only in the month of May and June. This suggests an use of heater in the winter months for the first household, whereas the second household uses both air conditioner in hot months and heater in cold months. Let us dive deeper and see the hourly usage across different days of the week for these two households.


```{r cust1_hd_my, fig.cap="(ref:cust1_hd_my)"}

library(ggplot2)
data_sm1 %>% granplot("month_year", "hour_day", response = "general_supply_kwh", plot_type = "quantile") +  ggtitle("Letter value plot of energy consumption across hours of day by month of years for Household 1")+ scale_y_continuous(breaks = seq(0, 1.5, 0.25))
```

(ref:cust1_hd_my-cap) Something that I am yet to write

```{r cust3_hour_day-month_year}
...
Hourly usage for two households across different days of the week are shown through decile plots. It is clear that energy conusmption only goes up for few hours in the evening. It goes much higher in the weekends implying that there are hardly any activities that take place during weekdays. Whereas, for the second household, there are two peaks - once during the day and one in the evening. Also, the difference between the energy consumption is not very different between weekdays and weekends. This suggests that the household might uniformly distribute their regular activity like using dish washer or heater/air conditioner throughout the week.


```{r cust1_hour-day_day-week}
data_sm1 %>% granplot("day_week", "hour_day", "general_supply_kwh", plot_type = "decile", quantile_prob = c(0.1, 0.25, 0.5, 0.75, 0.9)) +  ggtitle("Boxplot of energy consumption for Customer 1")
```
