...
The multiple order-up cyclic granularites resulting from the first two cases are circular and has been handled in \autoref{sec:singlefrommultiple} and \autoref{sec:all-circular}. The calendar categorization resulting from the last case are quasi-circular. Examples might include day of the quarter or hour of the month. Let us consider the computation of $A_{hour, month}$ given single order-up granularities $A_{hour, day} = 12$, $A_{day, month} = 20$ and $C_{month, year} = 9$.
...
\begin{split}
C_{day, year}(z) & = L(C_{day,month}(z) + \sum_{w=0}^{C_{month, year}-1}(\vert T_{w} \vert))
\end{split}
\end{equation}
