...
Drawing from the grammar of graphics, if <$A_i$, $B_j(s)$, $v(s)$> $i \in {1, 2, \dots, n}$ and $j \in {1, 2, \dots, m}$ serves as the basis of visualizing the distribution of the measured variable, the following layers can be specified:

- Data: <$A_i$, $B_j$, $v(s)$> $i \in {1, 2, \dots, n}$ and $j \in {1, 2, \dots, m}$ 
- Aesthetic mapping: $A_i$ mapped to $x$ position and $v(s)$ to $y$ position
- Statistical transformation: Any descriptive or smoothing statistics that summarizes distribution of $v(s)$
...
When we consider the layers of grammars considered for our data structure <$A_i$, $B_j$, $v(s)$> $i \in {1, 2, \dots, n}$ and $j \in {1, 2, \dots, m}$, mapping $A_i$ mapped to $x$ position and $B_j$ to facets
suggests that levels of $A_i$'s which are placed in close proximity are to be compared against each other and each level of $B_j$ represent a group. When $A_i$ mapped to facets and $B_j$ is mapped to $x$ axis, distributions are compared across levels of $B_j$ and levels if $A_i$ start acting as a group. However, the variables used in the layers are the same, the difference in mapping emphasizes different behavior of the variables.
