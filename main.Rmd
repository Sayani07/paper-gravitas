...
Mumbai Indians(MI) and Chennai Super kings(CSK) are considered one of the best teams in IPL with multiple winning titles. MI won against CSK both in 2013 and 2015 finals. It would be interesting to see the difference in their strategies throughout all matches in the two seasons. We can understand difference in strategy by answering these two questions. 
...
cricket_winner %>% as_tibble() %>% 
             filter(batting_team ==  "Mumbai Indians" | bowling_team == "Mumbai Indians",
                    ...
                    win_by_runs!=0) %>% select(win_by_runs) %>% summary()
...
cricket_winner %>% as_tibble() %>% 
             filter(batting_team ==  "Chennai Super Kings",
                    ...
                    win_by_runs!=0,
                    inning ==2) %>% select(win_by_runs) %>% summary()
```
...
In Figure \ref{fig:lv}, we can observe the letter value plot describing the distribution of run rate per over across over of the innings and innings of the match. It can be observed that for both innings, the run rate for Mumbai Indians rise faster across overs of the innings. There is an upward shift of the distribution in the second part of the first innings (after 10th over) and more variability (longer and distinct letter values) in the second part of 2nd innings. CSK has been more uniform in their approach maintaining consistent scores per over, whereas MI's scores increase as they proceed with the game notwithstanding if they bat first or second. CSK's run rate distibution is extremely right tailed in the 2nd innings, but not in the first. Since CSK's last over in the 2nd innings really get volatile, after a consistent performance in the other overs - this behavior is exhibited as an outlier in Figure \ref{fig:violin}.  
...
```{r lv, fig.cap = "Letter value plot of run rate across overs of an inning and innings of the match for (top) Mumbai Indians and (bottom) Chennai Super Kings."}
 ...
 cricket_tsibble_all %>% 
  filter(season %in% c(2010:2015), batting_team=="Chennai Super Kings",
         inning %in% c(1,2)) %>% 
           granplot("inning", "over",
                               hierarchy_model,
                               response = "run_rate",
