...
If we assume that linear granularities are integer sets with each integer representing a granule, we would be able to express the grouping operation through integer arithmetic which would pair better with the modular arithmetic used in the next section. The grouping operation Group$_m(G)$ essentially leads to a new granularity whose granules are the integer part of the quotient when the integer set spanned by linear granularity $G$ is divided by $m$ (the divisor), such that, $G_2(i) = \lfloor G_1(j)/m \rfloor$. While new granularities are generated with ordering guided by quotients, we shall see how for some cyclic granularities ordering is guided by the remainder with the assumption of linear granularities being represented by integer sets.
