...
Table \ref{tab:definitions} shows representation of circular granularities relating two linear granularities. It is possible that none of these two linear granularities are bottom granularities. But the representation of the resultant circular granularity will be a function of the bottom granularity. Suppose the bottom granularity is minutes, and $k_i$ is the number of granules of the finer granularity in each granule of the coarser granularity in the circular granularity $C_i$, also referred to as the period length.
...
Day-of-Month: & $QC_1(z)$ && $k_1=31, 30, 29, 28$\\
Hour-of-Month: & $QC_2(z)$ && $k_2=24*31, 24*30, 24*29, 24*28$\\
Day-of-Year: & $QC_3(z)$ && $k_3=366, 365$\\
Week-of-Month: & $QC_4(z)$ && $k_4=5, 4$\\ 
\bottomrule
\end{tabular}
\end{center}
...
\newtheorem{timedom1}{Definition}[subsection]
\begin{timedom1}\label{def:quasicircular}
A quasi-circular granularity QC_(BG, G') that relates linear granularities G' and bottom granularity BG, if

\begin{equation}\label{eq:eq3}
QC_{BG, G'}(z) = BG(z - \sum_{w=0}^{k-1}\vert T_{w \mod R'} \vert), z \in T_k
\end{equation}

where,  
z denotes the index set,  
BG denotes the index of the tsibble (bottom granularity),  
G' is a linear granularity periodic with respect to BG with irregular mapping,   
R' denotes the number of granules of G' in each period of the grouping (BG,G),   
T_w are the sets of indices of BG describing G'(w) such that $G'(w) = \bigcup_{z \in T_w}BG(z)$,$T_w = 0$ for $w \epsilon Z_{<0}$ and $\vert T_w \vert$ is the cardinality of set $T_w$.
\end{timedom1}
...
\begin{timedom}\label{def:acyclic}
