...
\autoref{fig:circular-dow} depicts the linear (a and b) and the corresponding stack (c and d) display of time. Circular granularities day-of-week $C_{B , G}$ and day-of-fortnight $C_{B, H}$ are constructed from linear granularities day ($B$), week ($G$) and fortnight ($H$). The granules that overlap share elements from the underlying time domain in the linear display. The circular granularities $C_{B , G}$ and $C_{B , H}$  <!--= C_{\text{day}, \text{week}}$ representing day-of-week and day-of-fortnight respectively --> consist of repeated pattern of categories $\{0, 1, 2, \dots, 6\}$ and $\{0, 1, 2, \dots, 13\}$ with $P=7$ and $P=14$ respectively. The granules that overlap represents the same category in the stack display. Each circular granularity can use descriptive label mappings, or be simply labelled using the index. <!--Similar to linear granularities, the indexing for each granule of the circular granularity can also be associated with textual representation, called the label.--> Suppose ${L}$ is a label mapping that defines an unique label for each index $l \in \{ 0,1,\dots, (P-1)\}$, then the label mapping $L$ for $C_{B, G}$ can be defined as
$$
...
This case is similar to Equation \ref{eq:multifromsingle-quasi2}. Single-order-up quasi-circular granularity ($C_{G_{l},G_{m'}}$) could be obtained from multiple-order-up quasi-circular granularity ($C_{G_l,G_{m}}$) and single/multiple-order-up circular granularity $C_{m',m}$ using \ref{eq:multifromsingle-quasi2}.
