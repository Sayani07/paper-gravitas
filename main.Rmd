...
where, 
$z \in \mathbb{Z}_{\geq 0}$ denotes the index set,
$T_{i_j}$ are the sets of indices of $B$ describing aperiodic linear granularities $M_{i}(j)$ such that $M_{i}(j) = \bigcup_{z \in T_{i_j}}B(z)$, $M = \bigcup_{j=1}^{n}M_{j}$ and index ${\{i_j\}}$ stands for the $j^{th}$ granule of the $i^{th}$ linear aperiodic granularity considered.
\end{definition}
...
**Discussion**: The number of categories obtained coincides with the number of aperiodic linear granularities considered and an additional category representing no event. 
...
C_{(G_l,G_m)}(z) & = L(C_{G_l,G_{l+1}}(z) + k(l, l+1)(C_{(G_(l+1),G_m)}(z))) \\  
  ...
  & =  L(C_{G_l,G_{l+1}}(z) + k(l, l+1)(C_{G_{l+1},G_{l+2}}(z) - 1) + k(l, l+1)k(l+1, l+2)(C_{G_{l+2}, G_{m}}))\\
  & =  L(C_{G_l,G_{l+1}}(z) + k(l, l+1)(C_{G_{l+1},G_{l+2}}(z) - 1) + k(l, l+2)(C_{G_{l+2},G_{l+m}}(z)))\\
  &\vdots\\
...
The grammar of graphics introduced a framework to incorporate underlying features for statistical graphics to construct them by relating data space to the graphic space [@Wilkinson1999-nk]. 
The layered grammar of graphics proposed by [@Wickham2009-pk], which is a modification over the former, suggests that graphics are made up of distinct layers of grammatical elements and meaningful plots are built around appropriate aesthetic mappings. 
<!-- seven grammatical elements in total, out of them three are essential.  -->

Briefly, the layered grammar defines the components of plot as:
...
- **Facet**ting is a mechanism for splitting the data into subsets, then plotting each subset in a different panel. This is a powerful tool for exploratory data analysis as we can rapidly compare patterns in different parts of the data and can see whether they are the same or different.
...
Drawing from the grammar of graphics, if <$A_i$, $B_j(s)$, $v(s)$> $i \in {1, 2, \dots, n}$ and $j \in {1, 2, \dots, m}$ serves as the basis of visualizing the probability distribution of the measured variable, the following components of the layered grammar can be specified:

- Data: <$A_i$, $B_j(s)$, $v(s)$> $i \in {1, 2, \dots, n}$
- Aesthetic mapping: $A_i$ mapped to $x$ position and $v(s)$ to $y$ position
- Statistical transformation: Any descriptive or smoothing statistics that summarizes distribution
- Geometric objects: e.g. boxplot, letter value, violin, ridge, highest density regions

After the essential layers, the next layer being used is the facet which dictates how to split up our plot.

- Facet:  $B_j(s)$



<!-- To add additional variables to a plot, we can use other aesthetics like colour, shape, and size or facetting. Facetting creates tables of graphics by displaying the same graph for each subset. -->
...
<!-- Aesthetics refers to the scales on to which we want to map our data. we create plots by layering grammarical elements on top of one another and use aesthetic maps -->
...
## Choice of exploratory visualization tool


<!-- Common benefits and challenges of empirical and density based visualization -->

There are several ways to visualize statistical distributions. The entire distribution can be displayed through density estimates using non-paramteric density estimation methods or only summarized values can be displayed using descriptive statistics. The latter do not allow us to see the shape, skewness, nature of the tail or multi-modality with so much clarity as the former. However, they avoid much clutter, where some specific probabilities representing typical and extreme behavior are on focus. Non-parametric density based plots can show the entire distribution, unlike discretizing and showing only parts of it. These displays depend on the specific estimation procedure (e.g., kernel density estimate) as well as on additional smoothing parameters and hence can vary for the same data set. Furthermore, these methods are subject to the same sample size restrictions and challenges that apply to any density estimation. In practice, the densities are estimated reasonably with at least 30 observations. However, even with large sample size, choosing too large a value for bandwidth can cause the estimate to oversmooth the data. The visualization tools based on descriptive statistics display only actual observations from the sample, and remain free of any tuning parameters or Kernels.
...
Quantile plots visually portray the quantiles of the distribution of sample data. These plots can display any quantiles instead of quartiles in a traditional boxplot. Some of the most implemented definitions of sample quantiles based on one or two order statistics can be found in @Hyndman1996-ty. These plots give more information than box plots and yet avoid clutter by enabling us to focus just on specific probabilities. When reviewing a boxplot, an outlier is defined as a data point that is located outside the fences (“whiskers”) of the boxplot (e.g. outside 1.5 times the interquartile range above the upper quartile and below the lower quartile). However, outliers are open to interpretation and not shown in an quantile plot.
...
### Non-parametric density based methods
...
The application is not only restricted to temporal data. We provide an example of cricket to illustrate how this can be generalized in other applications. The Indian Premier League (IPL) is a professional Twenty20 cricket league in India contested by eight teams representing eight different cities in India. In a Twenty20 game, an over will consist of 6 balls (with some exceptions), an innings is restricted to a maximum of 20 overs, a match will consist of 2 innings and a season can consist of several matches. The ball by ball data for IPL season 2008 to 2016 is fetched from [Kaggle](https://www.kaggle.com/josephgpinto/ipl-data-analysis/data). The `cricket` data set in the `gravitas` package summarizes the ball-by-ball data across overs and contains information for a sample of 214 matches spanning 9 seasons (2008 to 2016), each of which has 6 balls in an over and 2 innings in a match.
s
...
Although there is no conventional time granularity in cricket, we can still represent the data set `cricket` through a `tsibble`, with each over representing an ordering from past to future, can form the index of the tsibble. The hierarchy table would look like the following:
...
Mumbai Indians (MI) and Chennai Super kings (CSK) are considered one of the best teams in IPL with multiple winning titles and always appearing in final playoffs from 2010 to 2015. It would be interesting to take their example in order to dive deeper into the first question.
...
In Q2 and Q3, we considered dots balls per over or catches per over as aperiodic cyclic granularities. They do not appear in the hierarchy table since we can not compute these granularities as a function of circular or quasi-circular granularities. Also, their order in the hierarchy table is not clear as they are essentially obtained from some aperiodic linear granularity.
