...
What we need to add to this are additional categorizations of time that are not linear granularities and are useful to represent periodicity. Examples include day-of-week, time-of-day, week-of-year, day-of-month, month-of-year, workingday/non-workingday, etc. Many of these are circular, such as day-of-week, time-of-day. Some are nearly circular such as day-of-month. Some are irregular such as workingday/non-working day. Let's call all of these "calendar categorizations". Anything that maps a time index to a categorical variable can be considered a calendar categorization. We can specify the categorizations corresponding to circular granularities using modular arithmetic. The number of categories is essentially the periodicity of a circular time granularity.

For example, suppose the time index is in minutes, and let $n_i$ be the number of categories created by the circular granularity $C_i$. Then the following categorizations can be computed.
...
It is easy to note here that due to unequal length of months or years, these formulae can not be used for computing week-of-month or day-of-quarter. Thus, we start with the definition of circular granularity and then move on to computing aperiodic granularities. 
...
**Definition:** A **periodic circular granularity** $C$ with a modular period m is defined to be a mapping from the integers $Z$ (Index Set) to $Z_m$, such that $C(s) = (s\mod m)$ for $s \in Z$.
...
The index of a tsibble can be any date-time object which comes in different formats. [@Grolemund2011-qf] goes a long way in facilitating analysis of date-time data and extracting each date-time element with its own accessor function. However, table shows that it mostly allows for the creation of single-order-up granularities. 
...
Accessor = c("second()", "minute()", "hour()","wday()", "mday()","week()", "yday()", "month()")), format = "latex", booktabs = TRUE)
```

Hence, the idea is to create multiple-order-granularities from single-order-up in order to incorporate and be able to access different calendar categorisation. The computation of multiple-order-up granularities from single-order-up granularities can differ if single-order-up granularities are circular or aperiodic.

### All single-order-up granularities are periodic:

<!-- In section \@ref(cirgran-def), the definition of calendar categorisation considers index set as set of integers to be able to perform the modular operations. However, -->



Hence, this section will talk about creating multiple-order up granularities from these single-order-up granularities. The hierarchical structure of time helps to create a framework where the single-order-up granularities can be used recursively to create multiple-order-up granularities.

Consider a hierarchy table laying out the structure of all temporal units we are interested to look at, consisting of the following  three columns: 
...
Order: Order is defined as the position of the temporal units in the hierarchical table.
If the two granularties have three intermediate granularities in the hierarchical table, then their order difference is 3.

Suppose, $order(x) <  order(y)$  
$f(x, y)$ represents the functional form for computing granularity x_y
$c(x, y)$ is a constant which relates x and y. 

Then, 
$f(x, y) = f(x, x+1) + c(x, x+1)[f(x+1, y)-1]$, for $order(x+1) < order(y)$ can be used recursively to obtain any multiple order-up granularities. For $order(x+1) =  order(y)$, the function is same as the single-order-up granularities.
...
f(x, y) & = f(x, x+1) + c(x, x+1)(f(x+1, y)-1) \\  
  ...
  +i, x+i+1) - 1)\\
  & = \sum_{i=0}^{order(y) - order(x) + 1} c(x, x+i)(f_1(x+i) - 1)
\end{split}
\end{equation}

### Single-order-up granularities are mixed - that is circular or periodic:
...
```{r, hierarchy_tbl}
hierarchy_model <- tibble::tibble(units = c("minute", "hour", "day", "month", "quarter", "semester", "year"), conversion_fac = c(60, 24, NA, 3, 2, 2, 1))
