...
@Bettini1998-ed discusses the relationships between linear time granularities, the structure of a calendar, and periodicities in time.
...
We propose a formalism of nonlinear time granularities through the tsibble [@wang2019tsibble] framework of organizing temporal data. A tsibble consists of an index, key and measured variables. An index is a variable with inherent ordering from past to present and a key is a set of variables that define observational units over time. In a tsibble, each observation (row) should be uniquely identified by index and key.
...
Nonlinear time granularities with periodicities can be constructed by relating two linear granularities. The mappings between these linear granularities can be regular or irregular. For example, a regular mapping exists between minutes and hours (where 60 minutes always add up to 1 hour), while an irregular mapping exists between days and months (as the number of days per month ranges from 28 to 31). Mappings will be regular if linear granularities are formed by a grouping operation, but granularities formed by the altering-tick operation will be irregular. Nonlinear time granularities are referred to as **circular** if mappings between linear time granularities are regular and **quasi-circular** if the mapping is irregular. Examples of common circular granularities include hour of the day, and day of the week, whereas examples for quasi-circular granularities can be day of the month or week of the month.
...
\autoref{fig:circular-dow} depicts the circular granularity "day of the week", constructed from two linear granularities "day" (the bottom granularity) and "week". The granules that overlap share elements from the underlying time domain. In this example, $P=7$, and the circular granularity $C_{\text{day}, \text{week}}$ representing day-of-week consists of the repeated pattern $\{L(0), L(1), L(2),$ $L(3), L(4), L(5), L(6)\}$.
...
- $B \trianglelefteq G'$
- there exist $R'$, $P' \in \mathbb{Z}_+$ such that if $G'(w) = \bigcup_{z \in T_w}B(z)$ then $$G'(w) = \bigcup_{z \in T_w \mod R'}B(P'*\lfloor w/R' \rfloor + z)$$
