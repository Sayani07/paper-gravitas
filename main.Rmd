...
The basic plot choice for our data structure is one that can display distributions. For displaying the distribution of continuous univariate variable graphically, many options are available. Displays based on descriptive statistics include variations of box plots [@Tukey1977-jx] such as notched box plots [@McGill1978-hg] or quantile plots. Plots based on kernel density estimates include violin plots [@Hintze1998-zi], summary plot [@Potter2010-qc], ridge line plots [@R-ggridges], and highest density region (HDR) boxplots [@Hyndman1996-ft]. The less commonly used Letter-Value plots [@Hofmann2017-sg] is a midway between boxplots and density plots. Letter values are order statistics with specific depths. The median ($M$) divides the data set into halves. Each of the next steps splits the remaining parts into two separate regions so that the fourths ($F$), eighths ($E$), sixteenths ($D$) etc. are obtained. They display the distributions beyond the quantiles specially for large data,
