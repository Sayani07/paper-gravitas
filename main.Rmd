...
sm_cust50 %>%
  granplot("month_year",
    ...
    response = "general_supply_kwh", plot_type = "quantile", 
          quantile_prob = c(0.25, 0.5, 0.75), overlay = FALSE) + ggtitle("") +
...
There are many interesting questions that can possibly be answered with such a dataset, however, we will explore a few and understand how the proposed approach in the paper can help answer some of the questions.
...
First, we look at the distribution of runs per over across over of the innings and seasons in \autoref{fig:seas_over_inning}. The distribution of runs per over has not significantly changed from 2008 to 2016. There is no clear trend that we can observe for any over of the innings for any seasons. So we can work with any subsets of seasons and try to see how the strategies of the winning teams differ across seasons or how in a particular season the strategy was different for the winning team and the ones who did not qualify for the playoffs. 



```{r seas_over_inning}
 cricket_tsibble_all %>% granplot("over", "season",
                               hierarchy_model,
...
Mumbai Indians(MI) won against Chennai Super kings(CSK) in 2013 and 2015. So it would be interesting to see the difference in the strategies between these two teams tgroughout all matches in the two seasons. In \autoref{fig:2013_15_MI}, we can observe the letter value plot describing the distribution of run rate per over across over of the innings and innings of the match. It can be observed that for both innings, the run rate for Mumbai Indians rise faster across overs of the innings. CSK has been more uniform in their approach maintaining consistent scores per over, whereas MI's scores increase as they proceed with the game nothwithstanding if they bat first or second. CSK's run rate increase as it reached the last over when it bats in the 2nd innings, but doesn't increase much when it bats first.
...
```{r lv_MI_CSK}
# fig.cap = "Letter value plot of run rate across overs of an inning and innings of the match for (a) Mumbai Indians and (b) Chennai Super Kings.", fig.height=5.5, out.width="50%", fig.pos = "p", fig.align= 'left',echo=FALSE, eval=TRUE, fig.cap="Letter value plot of run rate across overs of an inning and innings of the match for (a) Mumbai Indians and (b) Chennai Super Kings.", fig.show = 'hold'
...
matches_select <- matches %>% select(id, toss_winner, winner)
...
CSK_winner <- cricket_winner %>% 
 ...
 MI_winner %>% 
            granplot( "win","inning",
                                ...
                                plot_type = "violin",
                                quantile_prob = c(0.1, 0.25, 0.5, 0.75, 0.9),
                     overlay = FALSE) + ggtitle("") + geom_boxplot(width = 0.1)
