...
**Definition: Finer Than** : A linear granularity G is finer than a  linear granularity H, denoted $G \preceq H$, if
...
$$C_{(G, H)}(z) = BG(\lfloor z/k(BG,G) \rfloor\mod k(G,H))$$,
...
### Computation of circular granularities
...
Definition:  **order** of a linear granularity in a hierarchy table H:$(G, l, m)$ is defined as the level of deconstruction of a linear granularity. In a hierarchy table, linear granularities are arranged from highest to lowest order of graininess. Thus the first linear granularity can either be the bottom granularity (finest possible) or the finest granularity within the set of linear granularities provided in the hierarchy table. 

Let H: $(G, l, m)$ be a hierarchy model containing m linear granularities. $$G_{i}$$ represents the linear granularity of order i and $$l_{i}$$å represents the period length of $$G_{i}$$ with respect to $$G_{i+1}$$.
...
\begin{split}
C_{(x,y)}(z) & = C_{(x,x+1)}(z) + k(x, x+1)(C_{(x+1,y)}(z)-1) \\  
  & =   C_{(x,x+1)}(z) + k(x, x+1)[ C_{(x+1,x+2)}(z) + c(x+1, x+2)( C_{(x+2, y)}(z) - 1) - 1]\\
