...
For example, suppose the time index is in minutes, and let $n_i$ be the number of categories created by $C_i$. Then the following categorizations can be computed.
...
We start with the definition of an equivalence class and then move on to define a circular granularity. 
...
**Definition:** A **aperiodic circular granularity** can not be defined in terms of modular arithmetic since each coarser unit is not always made up of equal number of finer units.
...
<!-- What is multiple and single order granularities -->
...
In section \@ref(cirgran-def), the definition of calendar categorisation considers index set as set of integers to be able to perform the modular operations. However, the index of a tsibble can be any date-time object which comes in different formats. [@Grolemund2011-qf] goes a long way in facilitating analysis of date-time data. It not only helps to identify and parse different date-time data, it also helps to extract each date-time element with its own accessor function. However, table 5 shows that it mostly allows for the creation of some calendar categorizations, usually single-order-up. 
