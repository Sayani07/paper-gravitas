...
and, **Order**, is defined as the position of the linear granularities in the hierarchical table. 

We refer to granularities which are nested within multiple levels as **multiple-order-up** granularities and those concering a single level as **single-order-up** granularities.

<!-- What is multiple and single order up granularities -->
 We take the help of Mayan calendar to illustrate this. In "Mayan" calendar, one day was referred to as 1 kin and the calendar was structured as follows: 
...
Suppose, z is the index set of the tsibble, x,y are two units in the hierarchy table  with  $order(x) <  order(y)$. Let $f_{(x, y)}$ denotes the accessor function for computing the single-order-up granularity x-of-y and $c(x, y)$ is a constant which relates x and y.
...
Then $f_{(x, y)}$ can be  computed using modular arithmetic as follows:
...
$kin-uinal:   = z \mod 20$  \\
$uinal-tun:   = \lfloor z/20\rfloor \mod 18 $\\
$tun-katun:    = \lfloor z/20*18\rfloor \mod 20 $\\
$katun-baktun:   = \lfloor z/20*18*20\rfloor \mod 20 $\\
