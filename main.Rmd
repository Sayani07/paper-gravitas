...
4. For each $i \in \{1, 2, \dots, l\}$, the maximum pairwise distance could be chosen from  ${m \choose 2}$ distances using $D_{i} = max(d_{jk})/(max(d_{jk}) - min(d_{jk}))$ $\forall j, k = {1, 2, \dots, m}$. Here, maximum is being adjusted against the range of the data to accommodate for comparison of maximum distances for different ranges.
