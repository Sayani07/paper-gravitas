...
```{r group-periodicial, echo=FALSE, out.width = "100%", fig.cap="Linear granularity $B$ groups periodically into linear granularity $G$ with $P = 5$ and $R = 2$, alternating between combining 2 and 3 granules of $B$."}
...
\todo[inline]{What is g-rel? - g-rel can be any granularity relationships like -   
groups into, finer than, groups periodically (the granularity relationships defined in this paper) or sub-granular, shift equivalent (not discussed in this paper but other relationships that the author discussed in @Bettini1998-ed)}
...
- **The grouping operation**: Let $G_1$ be a full-integer labeled granularity, and $m$ a positive integer. The grouping operation Group$_m(G)$ generates a new granularity $G_2$, by partitioning the granules of $G_1$ into $m$-granule groups and making each group a granule of the resulting granularity. More precisely, $G_2 = \text{Group}_m(G_1)$ is the full-integer labeled granularity such that for each integer $i$,
$$G_2(i) = \bigcup\limits_{j = (i-1)m+1}^{im} G_1(j).$$

\todo[inline]{What does "full-integer" mean? It might be the case that we don not always
have integers as the label sets of granularities. A full-integer labeled granularities are defined in definition 2 since labels coincide with indices. These labels can be an
arbitrary subset of (possibly noncontiguous) integers and could be used to identify granules. The author has introduced the definition of "labeled granularity" in @Ning2002-tf which could be added after definition of linear granularity to clarify this. }
...
\todo[inline]{What does this last phrase mean? - if  the time horizon is such that it starts from the 15th second of a day, then doing repeated operations of grouping might result in a granule of "resultant" granularity day or week, which is different from a time-stamped day or week. Could be phrased better or deleted.}


When the granules for second and minutes are arranged, the first granule of seconds and minutes granules should that overlap share elements from the underlying time domain}

- **The altering-tick operation** : Let $G_1$, $G_2$ be full-integer labeled granularities, and $l$, $k$, $m$ integers, where $G_2$ partitions $G_1$, and $1 \leq l \leq m$. The altering-tick operation $\text{Alter}^{m}_{l,k} (G_2, G_1)$ generates a new full-integer labeled granularity by periodically expanding or shrinking granules of $G_1$ in terms of granules of $G_2$. The altering-tick operation modifies the granules of $G_1$ so that the $l$th granule of each group has $|k|$ additional (or fewer when $k < 0$) granules of $G_2$.
...
\todo[inline]{Why is this called a ``pseudomonth''? - Here,  granularity pseudomonth is generated by grouping 31 days, and then shrink April, June, September and
November by 1 day, and shrink February (2) by 3 days. It doesn't incorporate adjustments for leap year or seconds in which case the period for the altering operation should be 400 instead of 12. ``pseudomonth`` computes months only in non-leap years.}
