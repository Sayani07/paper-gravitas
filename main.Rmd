...
# Nonlinear time granularities {#sec:cyclic-gran}
...
\begin{definition}[Aperiodic nonlinear granularity]\label{aperiodic}
...
```{r aperiodic-example, echo=FALSE,out.width = "100%",fig.cap="Aperiodic nonlinear granularity $A(B, M1)$ relating aperiodic linear granularity $M1$ and bottom granularity $B$"}
...
These are "structurally" empty combinations in that it is impossible for them to have any value of the measured variable. Let us take a specific example, where $C_1$ be day-of-month with 31 levels and $C_2$ be week-of-month with 5 levels. There will be $31\times 5=155$ sets $S_{ij}$ corresponding to the possible combinations of day-of-month and week-of-month. Many of these are empty. For example $S_{1,5}$, $S_{21,2}$, etc. This is also intuitive since the first day of the month can never correspond to fifth week of the month.
...
) + ggtitle("") + geom_boxplot(width = 0.5)
```
