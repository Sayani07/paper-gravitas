...
Periodicity is very common in all kinds of data. Circular granularities, which are constructed by using two linear granularities, can be utilized to explain such periodicities. However, mappings between linear granularities can be regular or irregular. For example, a regular mapping exists between minutes and hours, where 60 minutes always add up to 1 hour. On contrary, an irregular mapping exists between days and months, since one might can have days ranging from 28 to 31. Hence, circular granularities can also be "periodic" or "aperiodic" depending on if the mapping is regular or irregular.


Suppose we have a tsibble with a time index in one column and keys and variables in other columns.

A time domain, as defined by Bettini, is essentially a mapping of row numbers (the index set) to the time index.

A linear granularity is a mapping of row numbers to subsets of the time domain. For example, if the time index is days, then a linear granularity might be weeks, months or years.  Circular granularity, joining two linear granularities, therefore is a mapping between these two subsets of time domain. We use modular arithmetic to define circular granularity and use the following definitions:
...
Figure \ref{fig:hhd-dm} shows distribution of energy consumption across hour hour of the day across different days of the month. We can't see much variation across days of week implying that the energy behavior doesn't change depending on which day of the month it is.
...
#           quantile_prob = c(0.01, 0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99)) + ggtitle("") +
...
# 
# sm_cust50 %>%
#   prob_plot( "hhour_hour","hour_day",
#            response = "general_supply_kwh",
#            plot_type = "quantile", overlay = TRUE) + 
#   ggtitle("")



sm_cust50 %>%
  prob_plot( "hhour_day","day_month",
           response = "general_supply_kwh",
           plot_type = "quantile", overlay = TRUE) + 
  ggtitle("")
```
