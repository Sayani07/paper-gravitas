...
<!--existing closely related work and how ours relate --> 
 This paper utilises the nestedness of time granularities to obtain multiple-order-up granularities from single-order-up ones.
...
# Formal conceptualisation of calendar categorisations{#cirgran-def}

<!-- general calendar categorisation -->
Often we partition time into months, weeks, days and so on in a hierarchical manner to relate it to data. Such discrete abstractions of time can be thought of as time granularities [@aigner2011visualization]. Providing a formalism to this abstraction is important to model a time series across differently grained temporal domains. Examples of time abstractions may also include day-of-week, time-of-day, week-of-year, day-of-month, month-of-year, workingday/non-workingday, etc which are useful to represent periodicity in the data. Many of these are circular, such as day-of-week, time-of-day. Some are nearly circular such as day-of-month. Some are irregular such as workingday/non-working day. Let us call all of these different abstractions of time as "calendar categorizations".

These calendar categorisations can be linear, circular or aperiodic. We call the calendar categorisations as "linear" if they respect the linear progression of time. We call these _*linear time granularities*_. Examples include hours, days, weeks and months. _*Circular time granularities*_ accommodate periodicities in time such as hour of the day, and day of the week. _*Aperiodic circular granularities*_ are neither linear nor circular, such as day of the month or public holidays.
...
<!-- <!-- What is calendar categorisation --> 
...
**Defintion:** An **Aperiodic circular granularity** can not be defined using modular arithmetic in a similar fashion. The modulus for these type of calendar categorisations are not constant due to unequal length of some linear granularities. For example, please refer to the table below:
...
<!-- A time series which is characterised by being composed of repeating cycles. -->
Identifying repeating (periodic/aperiodic) patterns are necessary in revealing patterns and future trends of a temporal data. Often there is a need for periodicity detection to find whether and how frequent a periodic/aperiodic pattern is repeated within the series. To consider the exhaustive set of temporal regularities that might exist ein the data, we can categorise the set of temporal granularities to single or multiple order up granularities.
...
# Computation of multiple order-up granularities from single order-up granularities
...
```{r table_accessor}
kable(tibble::tibble(
  `Categorisation` = c("Second-of-minute", "Minute-of-hour", "Hour-of-day", "Day-of-week", "Day-of-month", "Week-of-year", "Day-of-year","Month-of-year"),
...
There can be three scenarios for obtaining calendar categorisation here:
- calendar categorisation consisting of two linear granularities whose orders are less than day  
- calendar categorisation consisting of two linear granularities whose orders are more than month  
- calendar categorisation consisting of one linear granularity with order at most day and another with order at least month  

The calendar categorisation resulting from the first two cases are periodic and has been handled in the earlier section.

The calendar categorisation resulting from the last case are aperiodic. Examples might include day of the quarter or hour of the month. In this section, we will see how to obtain aperiodic circular granularities of these types.
...
<!-- Suppose, we are not given the intermediate single-order-up granularities but still want all possible multiple order-up granularties. In that case, it is necessary to at least have the have the functional form that relates the smallest temporal unit to the highest temporal unit. In that case, we are able to form single-order-up from this functional relationship and still use the recursive relation to get the multiple order-up granularities. -->
...
### Computing single-order-up granularities:
