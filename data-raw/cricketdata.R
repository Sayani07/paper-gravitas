...
cricket_all_seas <- cricket_over_inning_crctd %>%
  filter(match_id %in% cricket_filtr_inning$match_id) %>%
  filter(wide_runs + noball_runs == 0) %>%
  mutate(ball_per_over = purrr::rep_along(match_id, 1:6)) %>%
  filter(inning %in% c(1, 2))

# Each season doesn't have equal number of matches- so season to be fed as a aperiodic element
# cricketdata %>% group_by(season) %>% summarise(n=length(unique((match_id)))

save(cricket_all_seas, file = "data/cricket_all_seas.rda")
