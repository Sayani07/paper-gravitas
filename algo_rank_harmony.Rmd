...
feature vectors are normally distributed, if the feature values are independent, identically distributed, and have limited variance. (“The Dissimilarity Representation for Pattern Recognition | Series in Machine Perception and Artificial Intelligence.” n.d. Accessed May 8, 2020. https://www.worldscientific.com/worldscibooks/10.1142/5965.)
We can not assume that feature values are independent.

  b) Gamma: (Ferencz, A., E. G. Learned-Miller, and J. Malik. 2005. “Building a Classification Cascade for Visual Identification from One Example.” In Tenth IEEE International Conference on Computer Vision (ICCV’05) Volume 1, 1:286–93 Vol. 1.) have considered the Gamma distribution to model the L2-distances.
...
```{r}
library(readr)
read_rds("data/smart_harmony_weibull.rds") %>% knitr::kable()
```
...
```{r}
read_rds("data/smart_harmony_gamma.rds") %>% knitr::kable()
```
...
```{r}
library(readr)
read_rds("data/smart_harmony_normal.rds") %>% knitr::kable()
```
...
## Distances follow Weibull
```{r}
read_rds("data/smart_cric_weibul.rds")%>% knitr::kable()
```
...
```{r}
library(readr)
read_rds("data/smart_cric_gamma.rds") %>% knitr::kable()
```

## Distances follow Normal 
```{r}
read_rds("data/smart_cric_normal.rds")%>% knitr::kable()
```
