...
```{r}
library(readr)
read_rds("data/smart_harmony_wb.rds") %>% knitr::kable()
```
...
```{r}
read_rds("data/smart_harmony_g.rds") %>% knitr::kable()
```
...
```{r}
library(readr)
read_rds("data/smart_harmony_norm.rds") %>% knitr::kable()
```
...
## Distances follow Weibull
```{r}
read_rds("data/ smart_cric_weibull.rds")%>% knitr::kable()
```
...
```{r}
read_rds("data/ smart_cric_general.rds")%>% knitr::kable()
```