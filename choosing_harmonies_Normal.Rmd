...
| ..               	|      	|      	|      	|    	|    	|      	|
| ..               	|      	|      	|      	|    	|    	|      	|
| d_m              	| d_m1 	| d_m2 	| d_m3 	| .. 	| .. 	| d_mL 	|

$H_{01}: d_{11} = d_{21} = \ldots = d_{m1} = 0$  
$H_{02}: d_{12} = d_{22} = \ldots = d_{L2} = 0$  
\vdots
$H_{0m}: d_{1m} = d_{2m} = \ldots = d_{Lm} = 0$  
...
```{r}
smart_meter10 %>%
  filter(customer_id %in% c(10017936)) %>% prob_plot( gran1 = "day_week", gran2 = "day_month", plot_type = "boxplot")


smart_meter10 %>%
  filter(customer_id %in% c(10017936)) %>% prob_plot(gran1 = "wknd_wday", gran2 = "day_month", plot_type = "boxplot")
